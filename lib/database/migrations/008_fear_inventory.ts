...
export async function up(db: SQLiteDatabase): Promise<void> {
  await db.execAsync(`
    ...
    INSERT INTO inventory_entries_new
      SELECT * FROM inventory_entries;

    DROP TABLE inventory_entries;

    ALTER TABLE inventory_entries_new RENAME TO inventory_entries;
...
export async function down(db: SQLiteDatabase): Promise<void> {
  // Remove any fear entries, then recreate table with original constraint
  await db.execAsync(`
    DELETE FROM inventory_entries WHERE type = 'fear';
    ...
    INSERT INTO inventory_entries_old
      SELECT * FROM inventory_entries;

    DROP TABLE inventory_entries;

    ALTER TABLE inventory_entries_old RENAME TO inventory_entries;
